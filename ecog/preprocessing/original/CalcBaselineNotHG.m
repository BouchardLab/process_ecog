function baselineD = CalcBaselineNotHG(subject,block,baselineTime,elects,frqs,method)

bdir = [subject '_B' num2str(block)];
ddir = [bdir '/HilbAA_4to200_40band'];
...
if exist([bdir '/' subject '_B' num2str(block) '_transcription_final.TextGrid'])
    ...
    [~,badsegs] = ParseTextGrid([bdir '/' transcription]); %extract times of events and remove bad times
...
for cnt1 = 1:length(elects)
    ...
    if strcmp(method,'silence')
        tmp1 = ceil(baselineTime(1)*fs);
        goodinds = goodinds(goodinds>tmp1);
        tmp1 = ceil(baselineTime(2)*fs);
        goodinds(goodinds>tmp1) = [];
    end
    goodinds(isnan(goodinds)) = [];  
    baselineD(1).mu_std(cnt1,:,1) = mean(tempdata(:,goodinds),2);
    baselineD(1).mu_std(cnt1,:,2) = std(tempdata(:,goodinds),[],2);

end
baselineBlock = bdir;
save([bdir '/baseline.forty.mat'],'baselineD','baselineTime','method','baselineBlock')
end
